...
port = 5670
agent_name = "Generateur_Elements"
...
# inputs
def Difficulty_Level_input_callback(io_type, name, value_type, value, my_data):
    try:
        agent_object = my_data
        assert isinstance(agent_object, Generateur_Elements)
...
if __name__ == "__main__":
    ...
    igs.definition_set_class("Generateur_Elements")
    ...
    agent = Generateur_Elements()

    igs.observe_agent_events(on_agent_event_callback, agent)

    igs.input_create("difficulty_level", igs.INTEGER_T, None)
    igs.observe_input("difficulty_level", Difficulty_Level_input_callback, agent)

    igs.output_create("element_array", igs.DATA_T, None)
