...
from Detecteur_Input_Snake_WL import *

port = 5670
agent_name = "Detecteur_Input_Snake_WL"
...
def print_usage():
    ...
    print("\nNOTE: This agent requires access to /dev/input/event* devices.")
...
def on_agent_event_callback(event, uuid, name, event_data, my_data):
    try:
        ...
        assert isinstance(agent_object, Detecteur_Input_Snake_WL)
...
if __name__ == "__main__":
    ...
    igs.definition_set_class("Detecteur_Input_Snake_WL")
    ...
    agent = Detecteur_Input_Snake_WL()
    ...
    igs.info("Waiting for arrow key input...")
    igs.info("(Press arrow keys to send direction commands)")
    
    while not is_interrupted:
        direction = agent.dir()
        if direction is not None:
            agent.DirectionO = direction
        elif is_interrupted:
            break
