...
# inputs
def Direction_input_callback(io_type, name, value_type, value, my_data):
    try:
        agent_object = my_data
        assert isinstance(agent_object, Moteur_Jeu)
...
def Pause_input_callback(io_type, name, value_type, value, my_data):
    try:
        agent_object = my_data
        assert isinstance(agent_object, Moteur_Jeu)
...
def Level_input_callback(io_type, name, value_type, value, my_data):
    try:
        agent_object = my_data
        assert isinstance(agent_object, Moteur_Jeu)
...
if __name__ == "__main__":
    ...
    igs.definition_set_class("Moteur_Jeu")
    ...
    igs.input_create("direction", igs.INTEGER_T, None)
    igs.observe_input("direction", Direction_input_callback, agent)
    igs.input_create("pause", igs.BOOL_T, None)
    igs.observe_input("pause", Pause_input_callback, agent)
    igs.input_create("level", igs.DATA_T, None)
    igs.observe_input("level", Level_input_callback, agent)

    igs.output_create("display", igs.DATA_T, None)
    igs.output_create("new_level", igs.IMPULSION_T, None)
    igs.output_create("game_over", igs.IMPULSION_T, None)
