...
from Generateur_Input_IA import *

port = 5670
agent_name = "Generateur_Input_IA"
...
def on_agent_event_callback(event, uuid, name, event_data, my_data):
    try:
        ...
        assert isinstance(agent_object, Generateur_Input_IA)
...
# inputs
def Manual_Control_input_callback(io_type, name, value_type, value, my_data):
    try:
        agent_object = my_data
        assert isinstance(agent_object, Generateur_Input_IA)
        agent_object.Manual_ControlI = value
        # add code here if needed
    except:
        print(traceback.format_exc())

def Display_input_callback(io_type, name, value_type, value, my_data):
    try:
        agent_object = my_data
        assert isinstance(agent_object, Generateur_Input_IA)
...
if __name__ == "__main__":
    ...
    igs.definition_set_class("Generateur_Input_IA")
    ...
    agent = Generateur_Input_IA()

    igs.observe_agent_events(on_agent_event_callback, agent)

    igs.input_create("manual_control", igs.BOOL_T, None)
    igs.observe_input("manual_control", Manual_Control_input_callback, agent)
    igs.input_create("display", igs.DATA_T, None)
    igs.observe_input("display", Display_input_callback, agent)
