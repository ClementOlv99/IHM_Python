...
from Generateur_Manuel import *

port = 5670
agent_name = "Generateur_Manuel"
...
# inputs
def Level_Number_input_callback(io_type, name, value_type, value, my_data):
    try:
        agent_object = my_data
        assert isinstance(agent_object, Generateur_Manuel)
...
if __name__ == "__main__":
    ...
    igs.definition_set_class("Generateur_Manuel")
    ...
    agent = Generateur_Manuel()

    igs.observe_agent_events(on_agent_event_callback, agent)

    igs.input_create("level_number", igs.INTEGER_T, None)
    igs.observe_input("level_number", Level_Number_input_callback, agent)

    igs.output_create("layout", igs.DATA_T, None)
